...
| **Operator console** (`GET /console`, `/console/data`) | **Solid (read-only console)** | Local dashboard: tails existing JSONL (decisions, escalations, router, debrief counts, observe snapshot, trust). Self-contained HTML (no CDN). **Never POSTs / never acts.** `make console` prints the URL. Sample: [EXAMPLE-CONSOLE.md](EXAMPLE-CONSOLE.md). |
