...
| Autonomy matrix (`src/governance/autonomy.py`) | **Solid** | Deterministic Risk×Trust×bylaw×evidence gate (ALLOW / REQUIRE_BRIEFING / ESCALATE / DENY); fail-safe default; tested. |
| Budget governor (`src/governance/budget_governor.py`) | **Solid** | Read facade over existing `GovernorConfig` thresholds + research paid-call cap; does not invent costs; tested. |
